...
and conceptual works that have influenced or are referenced in the nous design.

---

## Naming Convention for Inspired Abstractions

When a nous abstraction is directly inspired by a commercial hardware product,
the following convention applies:

- The **nous API name** is a generic descriptive term that accurately describes
...
- The **attribution.md entry** (this file) records the full product name, its
  owner, and which nous abstraction draws from it.

| Hardware product              | Owner                    | nous abstraction           |
|-------------------------------|--------------------------|------------------------------|
| Bloom step sequencer          | Qu-Bit Electronix        | `deffractal` / `nous.fractal` |
| Marbles random sampler        | Mutable Instruments      | `defstochastic` / `nous.stochastic`; `:stochastic-gate` / `:stochastic-cv` RT modulators |
| Labyrinth sequencer           | Moog Music Inc.          | `defflux` / `nous.flux`    |
| Mimeophon audio repeater      | MakeNoise                | `deftemporal-buffer` / `nous.temporal-buffer` |
| Cosmos drifting memory station | Soma Laboratory          | `deftemporal-buffer` / `nous.temporal-buffer` |
| Spectraphon spectral processor | MakeNoise                | `defspectral-resynthesizer` / `nous.spectral` |
| Panharmonium spectral oscillator | Rossum Electro-Music   | `defspectral-resynthesizer` / `nous.spectral` |
| GTE (Gestural Time Extractor)  | MakeNoise                | `defthreshold-extractor` / `nous.extractor`   |
| Solar 42 multi-voice drone synthesizer | Soma Laboratory          | `defpatch! :s42` / `nous.patch`               |
| Leviasynth 8-op FM synthesizer | ASM (Audiophile Synthesizer Machines) | `compile-fm :8op-cc` / `nous.fm` |
| Digitone 4-op FM synthesizer   | Elektron                 | `compile-fm :4op-cc` / `nous.fm`              |
...
Teenage Engineering AB. Referenced in nous design as examples of compact,
expressive performance devices.

### Qu-Bit Electronix Bloom
The Bloom (v2) step sequencer is a product of Qu-Bit Electronix. The nous
`nous.fractal` namespace and `deffractal` abstraction are directly inspired
...
"fractal" was chosen for the nous API to avoid trademark conflict.

### Arturia KeyStep Pro
The KeyStep Pro is a product of Arturia. Referenced in nous design for its
...
The BeatStep Pro is a product of Arturia. Referenced in nous design for its
...
(Émilie Gillet). The hardware and firmware source are MIT-licensed. The nous
`nous.stochastic` namespace and `defstochastic` abstraction are directly
...
"stochastic" was chosen for the nous API to avoid trademark conflict.

### Moog Labyrinth
The Labyrinth is a sequencer module produced by Moog Music Inc. (2024). The
nous `nous.flux` namespace and `defflux` abstraction are directly inspired
...
for the nous API to avoid trademark conflict.

### MakeNoise Mimeophon
The Mimeophon ("Stereo Multi-Zone Color Audio Repeater", 2019) is a product of
MakeNoise. The nous `nous.temporal-buffer` namespace and `deftemporal-buffer`
...
The name "temporal-buffer" was chosen for the nous API to avoid trademark conflict.
The R&R section covering this abstraction retains "(Mimeophon-Inspired)" as
explicit attribution.

### Soma Laboratory Solar 42
The Solar 42 is a product of Soma Laboratory. The nous `defpatch! :s42` patch
definition in `nous.patch` is directly inspired by the Solar 42's multi-voice
...
Memory Field preset within `nous.temporal-buffer` is directly inspired by the
...
### ASM Leviasynth
The Leviasynth is a product of ASM (Audiophile Synthesizer Machines). The nous
`compile-fm :8op-cc` backend in `nous.fm` is directly inspired by the
...
### Elektron Digitone
The Digitone is a product of Elektron. The nous `compile-fm :4op-cc` backend
in `nous.fm` is directly inspired by the Digitone's 8-algorithm topology
...
`nous.extractor` namespace and `defthreshold-extractor` abstraction are directly
...
The name "extractor" was chosen for the nous API to avoid trademark conflict.

### MakeNoise Spectraphon
The Spectraphon is a product of MakeNoise. The planned `nous.spectral` namespace
...
was chosen for the nous API to avoid trademark conflict.

### Rossum Electro-Music Panharmonium
The Panharmonium is a product of Rossum Electro-Music. The planned
`nous.spectral` namespace and `defspectral-resynthesizer` abstraction are
...
behavior at unity gain. The name "spectral" was chosen for the nous API to avoid
trademark conflict.

### Conductive Labs NDLR
The NDLR (Next Dimensional Loop Rider) is a product of Conductive Labs.
Referenced in nous design for its ensemble/voice model (Pad, Motif, Arp),
strum articulation, and harmonic context architecture. The `defensemble`
design in nous is informed by the NDLR's role model.
...
technology by Ableton AG, licensed GPL-2.0-or-later. nous optionally links
...
synthesis platform. The `ITemporalValue` protocol design in nous is
...
platform by Andrew Belt. The `ITemporalValue` protocol design in nous is
...
computational musicology developed at MIT, licensed BSD. nous uses Music21
as an optional backend for musicological analysis, score generation, and
corpus queries. The `python/nous_m21/` sidecar wraps the Music21 API.

### Overtone
Overtone (https://overtone.github.io/) is an open-source SuperCollider client
for Clojure. Referenced in nous DSL usability analysis as a comparable
live coding environment in the Clojure ecosystem.

### Sonic Pi
Sonic Pi (https://sonic-pi.net/) is an open-source live coding music
synthesizer by Sam Aaron, licensed MIT. Referenced in nous DSL usability
...
environment for pattern music by Alex McLean. Referenced in nous DSL
...
by Rich Hickey, licensed EPL-1.0. nous is implemented in Clojure.

### nREPL
nREPL (https://nrepl.org/) is an open-source Clojure network REPL, licensed
EPL-1.0. nous uses standard nREPL for interactive development.
...
and the MIDI Association. nous implements MIDI 1.0 output as a primary
target; MIDI 2.0 is a future consideration.

### OSC (Open Sound Control)
OSC is a communication protocol for musical instruments developed at CNMAT,
UC Berkeley. nous uses OSC as both a primary control plane and a data plane
...
systems. nous uses `.scl` and `.kbm` files for scale and keyboard mapping
definition (Q26).

### CLAP (CLever Audio Plugin)
CLAP (https://github.com/free-audio/clap) is an open-source audio plugin API.
nous targets CLAP for direct Eurorack CV output via the `nous-audio`
sidecar binary.
...
Bali). Referenced in nous design for its colotomic (stratified rhythmic
