# nous: Research and Requirements

## 1. Vision

`nous` is a music-theory-aware live coding sequencer implemented as a Clojure nREPL environment. It targets MIDI and Open Sound Control (OSC) outputs — controlling hardware synthesizers, software instruments, DAWs (such as Bitwig), Eurorack modular environments (via VCV Rack), and SuperCollider — without tight coupling to any single synthesis engine.

The system is intended for live performance and composition. The Clojure REPL is the primary interface: musicians define, modify, and compose sequencing behavior interactively, with changes taking effect in real time. The full power of a general-purpose functional language — persistent data structures, lazy sequences, transducers, macros, and the JVM ecosystem — means `nous` can express musical ideas and transformations well beyond what dedicated hardware sequencers can offer, while drawing heavily on the best concepts those instruments have developed.

**Ensemble synchronization** via Ableton Link is a first-class requirement. `nous` must be able to join a Link session and maintain beat/phase alignment with Ableton Live, Bitwig, Sonic Pi, and any other Link-enabled peer on the same LAN — enabling live performance with multiple musicians or hybrid hardware/software setups. See §15 for the full analysis and design.

### 1.1 Broader scope: a toolkit for novel musical instruments

`nous` is not only a live coding environment. It is a **platform for building novel,
theory-aware musical instruments, effects processors, sequencers, loopers, and
performance appliances**.

The unified control tree (§16) presents nous as a single logical device to any
external control surface — MIDI controller, TouchOSC layout, MCP client, or another
nous peer. Below that surface, the system fans out to multiple physical synthesis
...
- An embedded system (e.g. Raspberry Pi + touchscreen) running nous as headless
...
- A drum machine, MPC-style sampler, or Elektron-alike device, using a CLAP sampler
  plugin alongside nous's step sequencing and generative primitives
- An effects processor built from PureData's DSP capabilities combined with nous's
...
- A headless DAW controller, managing Ardour or Bitwig via their OSC interfaces
  while nous handles all sequencing and parameter modulation
...
The nous C++ sidecar implements the equivalent role: it is the GC-free, precise
...
```
libnous-rt (shared C++ static/dynamic library)
...
nous-sidecar  (links libnous-rt)    ← always running
├── MidiTarget  (RtMidi)
├── OscTarget   (UDP)
├── MIDI dispatch thread
├── Control-plane OSC server
└── IPC server  (Unix socket: /tmp/nous-sidecar.sock)

nous-audio    (links libnous-rt)    ← launched on demand
├── AudioEngine  (RtAudio)
├── ClapTarget
├── PdTarget     (libpd)
└── IPC server  (Unix socket: /tmp/nous-audio-N.sock)
```
...
```
JVM ──IPC──► nous-sidecar   (MIDI, OSC, Link timeline queries)
    ──IPC──► nous-audio-0   (CLAP/libpd plugin 0 — e.g., Surge XT)
    ──IPC──► nous-audio-1   (CLAP/libpd plugin 1 — e.g., Plugdata)
```
...
- **Scalability**: multiple `nous-audio` processes can run simultaneously,
  each hosting different plugins

**Build system**: CMake 3.25+. `libnous-rt` is built as a static library shared
by both binaries. Dependencies as git submodules:
- Ableton Link SDK (header-only, includes standalone Asio)
- RtMidi (used by `nous-sidecar`)
- RtAudio (used by `nous-audio`)
- libpd (used by `nous-audio`)

The Clojure startup sequence launches `nous-sidecar` unconditionally and optionally
launches one or more `nous-audio` instances as needed.
...
nous's control tree (§16) provides the address space; MIDI binding declarations map
CC numbers and note events onto tree paths.

```clojure
;; CC → value node
(ctrl/bind! {:midi/cc 74} "/nous/global/harmony/key"
  :transform (fn [v] (nth keys-in-fifths-order (int (* v 12/127)))))

(ctrl/bind! {:midi/cc 72 :midi/channel 16} "/nous/global/bpm"
  :transform (fn [v] (+ 60.0 (* v 180.0/127))))

;; Note-on → method node (Misha white-key interval model)
(ctrl/bind! {:midi/note-on 60 :midi/channel 16} "/nous/tone-row/advance"
  :args-fn (fn [vel] {:interval +1}))
(ctrl/bind! {:midi/note-on 62 :midi/channel 16} "/nous/tone-row/advance"
  :args-fn (fn [vel] {:interval +2}))

;; Note-on → trigger a live-loop restart
(ctrl/bind! {:midi/note-on 36 :midi/channel 16} "/nous/loops/bass/trigger")
```
...
- `nous` sends standard MIDI note messages; Rack handles D/A conversion to CV

**Via OSC (precision path)**:
- Install `trowaSoft cvOSCcv` or `OSC'elot` in VCV Rack
- `nous` sends float-precision OSC messages to Rack's OSC listener
...
**Misha as CV bridge**:
```
nous (MIDI) → Misha → 3× CV Out pairs → VCV Rack CV inputs
```
This uses Misha's interval/scale engine as a transformation layer. nous can set the anchor pitch via `PassThru/ActiveLastNote` mode, then let Misha's interval navigation drive the CV output.
...
2. **nREPL integration depth**: ~~Should `nous` extend nREPL with custom ops?~~
   **Resolved: standard nREPL with optional lightweight middleware; the control tree
   ...
   - **The control tree (§16)** is the canonical state model. All system state —
     ...
     `/nous/...` paths. Parameter inspection and real-time status are already served
   ...
   - **Optional `nous-nrepl` middleware**: a thin middleware layer can inject
     nous context into nREPL sessions (current BPM in the prompt, `ctrl/describe`
     for a selected path on demand) and provide a custom op
     `nous/watch path callback-op` that pushes tree-change notifications to the
...
8. **Microtonality in polyphony**: With standard MIDI, polyphonic microtonality requires one channel per note (or MPE). How many simultaneous microtonal voices can `nous` support practically? 16 channels = 16 voices max.
...
1. Define the core namespace structure (`nous.time`, `nous.pitch`, `nous.scale`, `nous.rhythm`, `nous.midi`, `nous.osc`, `nous.seq`, `nous.harmony`, `nous.tuning`, `nous.analyze`)
...
Ableton Link is a first-class requirement for `nous`. This section provides a
comprehensive analysis of the protocol, JVM integration options, and the design
for aligning Link's beat timeline with nous's virtual time model.
...
Because `nous` uses a native C++ dispatch sidecar (§3.5), the Link SDK is
...
**Architecture:**
```
nous (JVM) ──Unix socket──► nous-sidecar (C++) ──Link SDK──► Link session (LAN)
                                       │
...
The central design challenge: `nous`'s `sleep!` computes its target wall time from
...
The Clojure `nous.link` namespace communicates with the Link engine in the sidecar
...
```clojure
;; nous.link — Link integration via C++ sidecar IPC
(ns nous.link
...
The `nous.time/sleep!` implementation checks `(link/active?)` and branches at
...
1. **Sidecar Phase 1** (with core timing): Link SDK as submodule. `LinkEngine` class
   ...
   Verify: connect two machines running nous, confirm beat alignment < 1ms.
...
and made first-class in nous, resolves all four problems above.
...
```
/nous/global/bpm           :float  [20.0 300.0]  default: 120.0
/nous/global/harmony/key   :keyword              default: :C
/nous/global/harmony/mode  :keyword              default: :major
/nous/loops/bass/velocity  :int     [0 127]       default: 100
/nous/loops/bass/mute      :bool                 default: false
```

**Method node** — a callable Clojure function with a declared argument schema.

```
/nous/loops/bass/trigger   fn()         → start or restart the bass loop
/nous/loops/bass/stop      fn()         → stop the bass loop
/nous/tone-row/advance     fn(interval) → step the tone row by interval
/nous/harmony/next-chord   fn()         → advance chord progression one step
```
...
```
OSC:  /nous/global/harmony/key "G"  →  (ctrl/set! "/nous/global/harmony/key" :G)
OSC:  /nous/global/bpm 140.0        →  (ctrl/set! "/nous/global/bpm" 140.0)
OSC:  /nous/loops/bass/trigger      →  (ctrl/call! "/nous/loops/bass/trigger")
```
...
reaches every `/nous/loops/*/mute` node.
...
reflects into the tree node `/nous/global/bpm`. This is the same mechanism as MIDI
...
- `/nous/loops/bass` — container
- `/nous/loops/bass/velocity` — value node, backed by an atom
- `/nous/loops/bass/pattern` — value node
- `/nous/loops/bass/mute` — value node
- `/nous/loops/bass/trigger` — method node: start/restart
- `/nous/loops/bass/stop` — method node: stop
- `/nous/loops/bass/status` — value node: `:running | :stopped | :overrun`
- `/nous/loops/bass/beat` — value node: current beat counter (read-only, updated by loop)

The loop body calls `(ctrl/get "mute")` using a *path relative to its own subtree
root*. Absolute paths work too: `(ctrl/get "/nous/loops/bass/mute")`. Short
...
```clojure
(deflive-loop "/nous/loops/melody"
...
;; Now a MIDI CC change to /nous/loops/melody/key takes effect on next iteration
...
(binding [*bpm*     (or @link-bpm (ctrl/get "/nous/global/bpm"))
          *harmony* (ctrl/get "/nous/global/harmony")]
  (loop-body-fn))
```
...
```clojure
;; List all nodes
(ctrl/ls "/nous")            ; → #{"/nous/global" "/nous/loops" "/nous/tone-row"}
(ctrl/ls "/nous/loops")      ; → #{"/nous/loops/bass" "/nous/loops/drums"}

;; Read a node's full description
(ctrl/describe "/nous/loops/bass/velocity")
;; → {:path "/nous/loops/bass/velocity"
;;    :type :int
;;    :range [0 127]
;;    :default 100
;;    :current 85
...
(ctrl/snapshot)
;; → {"/nous/global/bpm" 138.0
;;    "/nous/global/harmony/key" :G
;;    "/nous/loops/bass/velocity" 85
;;    ...}
...
bindings. This is the nous equivalent of a hardware synthesizer's patch system.

### 16.7 OSC Namespace Query Support (OSC 1.1)

OSC 1.1 defines a query mechanism via the `#bundle` special address. When a client
sends an OSC query to `/_osc`, the server responds with its namespace. The nous OSC
...
controller software to *autodiscover* what nous exposes without manual configuration.
...
(ctrl/defdevice "/nous/devices/prophet"
...
This creates:
- `/nous/devices/prophet/filter-cutoff` — a value node; writing it sends MIDI CC 74
  to the Prophet on channel 1 via the sidecar
- `/nous/devices/prophet/resonance` — similarly
- A Clojure function `(play-note! "/nous/devices/prophet" pitch vel)` that routes
...
```clojure
(mod-route {:source "/nous/global/harmony/degree"   ; input: current chord degree
            :dest   "/nous/devices/prophet/filter-cutoff"  ; output: CC 74
...
(`/nous/**`). A configurable prefix or separate UDP port keeps them separate.
...
via ordinary function composition. A handler that controls the nous tree looks
...
- HTTP `GET /nous/global/bpm` → read the value at that path
- HTTP `PUT /nous/global/bpm` body `140` → write the value
- HTTP `POST /nous/loops/bass/trigger` → call the method
- OSC `/nous/global/bpm 140.0` → write the value (implicit set)
- OSC `?/nous/global/bpm` → query the value (OSC 1.1 prefix convention)
- OSC `/nous/loops/bass/trigger` (no args) → call the method
...
{:ctrl/path    "/nous/global/bpm"   ; tree path (canonical)
...
;; /nous/loops/*/mute → dispatch to all matching paths
...
(ns nous.server.http
  (:require [aleph.http :as http]
            [nous.server.handler :refer [control-stack]]))
...
```javascript
const ws = new WebSocket("ws://localhost:7000/ws/nous/global/bpm");
...
Because every tree node is a URL, the running nous system becomes navigable via
a browser. With content negotiation (`Accept: text/html`), the HTTP handler renders
tree nodes as HTML pages:

```
GET /nous/loops                          → HTML: list of all loops + status
GET /nous/loops/bass                     → HTML: bass loop parameters + controls
GET /nous/loops/bass/velocity            → HTML: slider + current value
GET /nous/global/harmony                 → HTML: key/mode/degree selector
GET /nous                                → HTML: full system dashboard
```
...
`GET /nous?format=edn` returns the full system state in one request.
...
(ns nous.server
  (:require [aleph.http  :as http]
            [aleph.udp   :as udp]
            [nous.server.http  :refer [http-handler ws-handler]]
            [nous.server.osc   :refer [start-osc!]]))
...
The control tree (§16) already models nous as a namespace of readable values and
...

{
  ...
  "inputSchema": {
    "type": "object",
    "properties": {
      "path":  {"type": "string", "description": "Tree path, e.g. /nous/global/bpm"},
...
{
  "name": "repl_eval",
  "description": "Evaluate Clojure code in the live nous nREPL session. Use with care — this executes arbitrary code.",
  "inputSchema": {
    "type": "object",
...
```clojure
;; Defined in nous.server.mcp/prompts
[{:name        "compose-progression"
 ...
 {:name        "analyze-state"
  :description "Describe what nous is playing right now in musical terms"
  :arguments   []}
...
(ns nous.server.mcp
  (:require [aleph.http :as http]
            [manifold.stream :as s]
            [nous.ctrl :as ctrl]
            [nous.server.mcp.tools :as tools]
            [nous.server.mcp.resources :as resources]))
...
tier can be set per client ID in nous config.
...
output). The `SynthTarget` abstract class lives in `libnous-rt` and is implemented
by both `nous-sidecar` (for non-audio targets) and `nous-audio` (for
...
(play-note! "/nous/synths/surge"   :C4 100)
(play-note! "/nous/synths/prophet" :C4 100)  ; MIDI hardware
(play-note! "/nous/synths/sc"      :C4 100)  ; SuperCollider via OSC
(play-note! "/nous/synths/pd"      :C4 100)  ; Pure Data via libpd

;; live-loop targeting a synth
(live-loop :lead
  (with-target "/nous/synths/surge"
...
CLAP is the primary target for nous plugin hosting. The MIT SDK license, C API
...
**`nous-audio` process** (§3.5), not in `nous-sidecar`. The audio process is
launched on demand by the Clojure side:

```sh
# Launched by nous.audio/start! in Clojure — not by the user directly
nous-audio --rate 48000 --buf 256
nous-audio --rate 44100 --buf 128     # lower latency
nous-audio --device "BlackHole 2ch"   # virtual device → DAW
```
...
```cpp
static const clap_host_t nous_host = {
    .clap_version = CLAP_VERSION_INIT,
    .host_data    = nullptr,
    .name         = "nous",
    .vendor       = "nous",
    .url          = "https://github.com/nous/nous",
    .version      = "0.1.0",
...
(ctrl/set! "/nous/synths/surge/tuning" "partch_43")
...
`/nous/synths/surge/params/`. Because 600 tree nodes is unwieldy, a lazy
...
(ctrl/register-all-params! "/nous/synths/surge")

;; Lazy default: register named parameters as needed
(ctrl/register-param! "/nous/synths/surge" "filter_cutoff_0")
;; → creates /nous/synths/surge/params/filter-cutoff-0 as a value node

;; Bind by CLAP parameter ID directly (no tree node required)
(ctrl/bind! {:midi/cc 74} {:plugin "/nous/synths/surge" :param-id 107})
```
...
;; Auto-registers:
;; /nous/synths/pd/params/cutoff    (value node)
;; /nous/synths/pd/params/resonance (value node)
;; /nous/synths/pd/params/waveform  (value node)
;; /nous/synths/pd/methods/trigger  (method node → libpd_bang)

;; Hot-swap the patch:
(ctrl/call! "/nous/synths/pd/load-patch" "bass-synth.pd")
```
...
**`nous-sidecar` threads (unchanged from §3.5):**
...
|---|---|---|
| `nous-sidecar` | Wall-clock ns | `clock_nanosleep` in MIDI dispatch thread |
| `nous-audio` | Sample position | Audio callback window conversion |
...
The philosophy is that nous should not reimplement what Music21 already does well.
The analysis and corpus functionality sketched in §10 and §11 can be largely backed by
Music21. nous provides the performance-critical real-time path; Music21 provides the
...
### 20.2 Capabilities Relevant to nous
...
`(m21/parse "path/to/score.xml")` returns a nous score map; `(m21/export score
...
┌───────────────────────────▼─────────────────────────────────────┐
│  nous-m21-server (Python process)                             │
│  - asyncio event loop                                           │
│  - Unix socket server                                           │
...
3. **Shared type vocabulary**: communication uses JSON-serialized maps with
   field names that match nous's keyword conventions (`:pitch/midi`, `:dur/beats`,
   etc.), making round-trips transparent.

4. **Optional**: the Music21 sidecar is not required for core playback functionality.
   `nous.m21` degrades gracefully when the sidecar is not running — analysis
...
crosses the IPC boundary, and its equivalent nous Clojure type.

| Concept | Music21 Python | JSON wire | nous Clojure |
|---|---|---|---|
...
### 20.6 Clojure Namespace: `nous.m21`
...
The critical question is: when does nous compute locally, and when does it delegate
to Music21? The rule is simple: **performance path = nous; analysis path = Music21**.

| Concern | nous native | Music21 sidecar | Notes |
|---|---|---|---|
...
| Chord spelling from Roman numeral | Yes (basic) | Yes (full) | nous handles playback cases; Music21 handles figured bass |
| Key analysis | Heuristic only | Yes — full K-S | Music21 result used to update `*key*` binding |
| Roman numeral parsing | Partial | Yes | nous parses `:I :IV :V`; Music21 handles secondary dominants, suspensions |
| Voice leading check | No | Yes | Pre-composition check; not on hot path |
| Tone row prime/retrograde/inversion | Yes (pure functions) | Yes (handles all 48) | nous handles common cases; Music21 for matrix and Z-relations |
...
async def main():
    socket_path = sys.argv[1] if len(sys.argv) > 1 else "/tmp/nous-m21.sock"
...
packages beyond Music21 and the standard library. The Clojure `nous.m21` namespace
...
**No Music21 corpus licensing concern for distribution**: nous does not bundle any
corpus files. Users install Music21 via pip, which includes the corpus under its own
terms. nous only provides the `corpus.search` / `corpus.parse` RPC calls that
...
`nous.m21` rather than standalone implementations. This gives §10 and §11 full
...
With Music21 tools exposed via MCP, an AI assistant connected to nous can:
...
The nous MCP server (§18.3) dispatches `tools/call` requests for `m21_*` tool names
to the Music21 sidecar via the existing `nous.m21` IPC client rather than
...
Western common-practice theory dominates most music software. nous should be equally
...
nous ships a `resources/scales/maqam/` directory with `.scl` files for all
common maqamat.

#### 21.2.3 Modulation (Intiqal)

In performance, maqam can shift to a related maqam — typically sharing a jins at
the ghammaz. For example, Rast can modulate to Bayati at its fifth. nous represents
...
These are implemented as named patterns in the `nous.rhythm` namespace (§22):
...
Since every gamelan has unique tuning, nous must support **per-ensemble tuning
...
the bonang plays 4 notes per balungan note; in irama dadi, 8 notes. nous
...
length of its gongan. nous expresses this as a long-period `live-loop` or via
...
nous provides named timeline patterns:
...
in successive bars. nous expresses this as consecutive `sleep!` durations:
...
combining chord voicings with the syncopated clave rhythm. In nous, this is an
...
based on syllabic delivery of text. Less relevant to nous's sequencing model
...
nous ships a bundled scale library covering non-Western tunings. It is separate from
...
(param-loop :filter-step
  :target "/nous/synths/surge/vcf/cutoff"
  :rhythm (euclid 5 16)
...
(param-loop :transpose-seq
  :target "/nous/loops/melody/transpose"
...
(param-loop :filter-a :target "/nous/synths/lead/vcf/cutoff"
  :rhythm :continuous :values (lfo :sine :period 4 :lo 300 :hi 5000 :phase 0) :dur 4)
(param-loop :filter-b :target "/nous/synths/bass/vcf/cutoff"
...
(param-loop :filter :period 5
  :target "/nous/synths/lead/vcf/cutoff"
  :rhythm (euclid 3 10)
...
The resulting `.edn` file is bundled with nous. At runtime:
...
In addition to corpus-derived patterns, nous ships a library of named arpeggios:
...
with a fixed rhythmic pattern and optional octave variation. nous generalizes this
...
All of nous's time-domain control descends from a single abstract concept:
...
nous can absorb several of Marbles' conceptual mechanisms as first-class primitives:
...
be synchronized. This maps directly onto a distinction nous already makes:

| Marbles | nous equivalent |
|---|---|
...
The critical observation: in nous, a `live-loop` already implicitly couples rhythm
...
In nous, jitter is implemented as a perturbation on the `sleep!` duration:
...
(ctrl/set! "/nous/loops/melody/deja-vu" 0.85)
;; Later: dissolve back to randomness
(ctrl/set! "/nous/loops/melody/deja-vu" 0.1)
```
...
Bringing the complete Marbles model together, nous provides a `defstochastic` macro that
...
`/nous/stochastic/<name>/`, making all parameters addressable from MIDI CC, OSC,
HTTP, and MCP.
...
| Marbles mechanism | nous primitive | Key parameters |
|---|---|---|
...
Bloom introduces several concepts that nous does not yet have:
...
The branch type is determined by the module's internal algorithm; in nous, the
...
nous, `bloom/build-tree` is a pure function — call it whenever the trunk changes.
The control tree (§16) holds both the trunk and the regeneration trigger:

```clojure
(defnode "/nous/bloom/melody/trunk" :data)
(watch! "/nous/bloom/melody/trunk"
  (fn [_ _ _ new-trunk]
...
In nous, a path is a sequence of branch indices:
...
In nous, mod data is added to the step map:
...
parameters in the control tree at `/nous/fractal/<name>/`, making Branch count, Path,
...
(ctrl/bind! {:midi/cc 74} "/nous/bloom/my-bloom/root"
...
mod value/mode) motivates a **unified step map** for nous that all sequencing
primitives understand:

```clojure
;; The canonical nous step map
...
All public symbols are in `nous.flux`. Import with:

```clojure
(require '[nous.flux :as flux])
```
...
All sugar is implemented as macros or functions in `nous.dsl` and re-exported
from `nous.core`. The underlying protocol-level API (`play!` with a full step map,
...
```clojure
;; Returns an infinite cycling seq with a nous ring type that tracks position
...
All sugar forms live in `nous.dsl` and are re-exported from `nous.core` via
...
As an `ITemporalValue` (defined in `nous.clock`):
...
### 28.3 Core operation vocabulary (`nous.phasor`)
...
### 28.4 Shape functions (`nous.phasor`)
...
nous.clock    — requires nous.phasor
       ...
       ↑
nous.mod      — requires nous.clock + nous.phasor
...
nous.timing   — requires nous.clock + nous.phasor
                  swing, humanize, groove templates
                  All return Long nanosecond offsets (Q35, Q46)
```

`nous.phasor` has no Clojure protocol or record dependencies — it is pure
arithmetic and can be used independently. `nous.clock` builds the protocol layer
...
### 28.7 LFOs and envelopes (`nous.mod`)
...
| `nous.phasor` as a no-dependency namespace | Pure math; usable independently; shapes compose cleanly |
| `ITemporalValue` and `Phasor` co-located in `nous.clock` | Avoids circular dep between protocol and implementation namespaces |
| `Lfo` moves from `nous.clock` to `nous.mod` | `nous.clock` is clock infrastructure; LFOs are modulation signals |
| `Swing` moves from `nous.clock` to `nous.timing` | Timing modulators have their own namespace (Q46) |
...
Time is the most fundamental concern of a sequencer, and the most treacherous. nous
...
| **MIDI Clock** | MIDI sync | 24 pulses per quarter note | nous (master) or external (slave) |
| **MIDI Time Code** | Wall-clock (SMPTE) | HH:MM:SS:FF | nous (master) or external (slave) |
...
nous adopts this convention: `sleep! 1` advances by one quarter note, regardless of
...
and conductors typically mean dotted-quarter BPM in 6/8. **nous follows the MIDI
...
A 7/8 bar is typically grouped as either 3+2+2 or 2+2+3 eighth notes. nous
...
Internal beat positions in nous are exact Clojure rationals. Every PPQN resolution
...
nous must be capable of exporting at any of these PPQN values by quantising
...
A nous `start!` when MIDI Clock output is enabled sends Start + begins Clock
...
#### Reception (nous as MIDI Clock slave)
...
applications and devices. It is embedded in the nous C++ sidecar.
...
- When no peers are connected, nous's local BPM is authoritative

#### Phase quantization

Link's `RequestBeat` mechanism: when nous starts its transport or a loop wants to
...
The nous-audio sidecar processes audio at a **sample rate** (typically 44100 or
...
nous-audio sidecar must deliver MIDI events at a specific **sample offset within the
...
If the studio master clock is at 48000 Hz and nous's audio interface is at 44100 Hz
...
96 PPQN (for higher resolution sync). Should nous allow the output PPQN to be
...
samples exactly). 29.97 drop-frame is required for NTSC video sync. Should nous
...
wired together as a virtual effects rack or pedalboard, with nous's sequencing and
modulation system as the control layer.

### 30.1 The Concept

A **plugin rack** in nous is a directed signal flow graph where:
...
and DAW insert chains — but in nous it is defined in Clojure data, live-codeable
at the nREPL, and directly controlled by `deflive-loop`, `defstochastic`, `defflux`,
and all other nous sequencing primitives.
...
IR loader — wired in series. The rack runs in `nous-audio`. Parameters (delay time,
...
system: oscillator (CLAP synth) → filter (CLAP effect) → envelope (nous `one-shot`)
→ VCA (gain node) → reverb (CLAP effect). The LFO is a nous `lfo` bound to the
...
at the REPL, serialisable to EDN, and sent to `nous-audio` via IPC for realisation.
...
The graph is sent to `nous-audio` as an IPC command. The audio process
...
The plugin graph in `nous-audio` must handle:
...
### 30.7 Integration with nous Modulation and Sequencing

The plugin rack is not a passive signal chain — it is a **live-modulated performance
instrument**. Every plugin parameter is a leaf node in the nous control tree,
...
This means every nous modulation primitive applies directly:

| nous primitive | Plugin rack use |
|-----------------|----------------|
...
The plugin rack exposes its parameters to the nous modulation system via `defdevice`
...
|-------------------|---------------------|
| `nous-audio` (§3.5) | The process that hosts and runs the plugin graph |
...
| `nous.phasor` (§28) | LFO sources driving plugin parameters |

The primary new component is the `defrack` macro and the IPC protocol for
communicating graph topology to `nous-audio`. Everything else is already in the
design.
...
nous's plugin rack is **code-first**: the graph is defined in Clojure data, wired
...
| Dimension | Gig Performer | nous plugin rack |
|-----------|--------------|---------------------|
...
cutoff pattern. These are idiomatic nous expressions, not special plugin-rack
features.
...
### 31.2 Value Assessment for nous

**High value — export path (nous → DAW)**:

The primary workflow is "sketch in nous, finish in a DAW." A live-coded session
...
This is the "sketch-to-DAW" workflow: nous as a generative composition and
performance tool that hands off to Bitwig, Studio One, or Cubase for production.

**Medium value — import path (DAW → nous)**:
...
- **Tempo map**: import BPM automation and time signature changes from a video project
  or collaboration; ingest into nous's system-state timeline (§29.9)
...
- **Plugin states**: load a plugin's `.dawproject` state blob into a running
  `nous-audio` session — equivalent to loading a preset saved in Bitwig
...
representation of a nous session.
...
nous-relevant subset with no added dependency and is straightforward to write from
the published XSD schemas.

### 31.3 nous Concept → DAWProject Mapping

| nous concept | DAWProject equivalent |
|---------------|----------------------|
...
§29.9 master timeline design — a nous session's time configuration serialises
naturally.
...
the `envelope` breakpoints from §28.7 — nous's piecewise-linear envelopes can be
...
the ZIP, referenced by path from `project.xml`. The `nous-audio` sidecar can
...
duration) maps directly to the nous step map (`:pitch/midi`, `:dur/beats`,
`:gate/on?`, `:mod/value` for velocity). The time values in DAWProject are in beats
(quarter notes) — same anchor as nous's rational beat positions (§29.2).

**Bitwig as the primary interchange target**: Bitwig Studio and nous share two
foundational choices — CLAP as the primary plugin standard and Ableton Link for
network sync. A nous session exported to DAWProject will have the best fidelity
when imported into Bitwig, including CLAP plugin states.

### 31.5 Design Questions for the Sprint

**Q66 — Export scope: what nous constructs should export to DAWProject?**
...
nous constructs (or producing a `defrack` form from the DAWProject's plugin chain).
